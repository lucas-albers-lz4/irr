...
import (
	...
	"github.com/lalbers/helm-image-override/pkg/chart"
...
func main() {
	rootCmd := &cobra.Command{
		Use:   "helm-image-override",
		Short: "Tool for generating Helm overrides to redirect container images",
		Long: `helm-image-override helps migrate container images between registries by generating
Helm value overrides that redirect image references to a new registry.`,
	}
...
to a target registry. This is the original functionality of helm-image-override.`,
		RunE: runDefault,
	}
...
func runDefault(cmd *cobra.Command, args []string) error {
	...
	if pathStrategy != "prefix-source-registry" {
		fmt.Fprintf(os.Stderr, "Error: Unsupported path strategy: %s\n", pathStrategy)
		fmt.Fprintln(os.Stderr, "Currently only 'prefix-source-registry' is supported")
		return fmt.Errorf("unsupported path strategy")
	}
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error loading chart: %v\n", err)
	...
	if registryMappingsFile != "" {
		...
		if err != nil {
			fmt.Fprintf(os.Stderr, "Error loading registry mappings: %v\n", err)
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error getting path strategy: %v\n", err)
	...
	if verbose || debugEnabled {
		...
		if err != nil {
			fmt.Fprintf(os.Stderr, "Error detecting images: %v\n", err)
	...
	overrides, err := chart.GenerateOverrides(chartData, targetRegistry, sourceRegistriesList, excludeRegistriesList, strategy, verbose)
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error generating overrides: %v\n", err)
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error converting overrides to YAML: %v\n", err)
	...
	if !dryRun {
		if err := validateHelmTemplate(chartPath, yamlData); err != nil {
			fmt.Fprintf(os.Stderr, "Error validating generated overrides: %v\n", err)
			fmt.Fprintf(os.Stderr, "This may indicate an issue with the chart's templates or values.\n")
			fmt.Fprintf(os.Stderr, "Try running 'helm template %s -f <override-file>' manually to see the full error.\n", chartPath)
