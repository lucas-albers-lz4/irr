...
func createTempChartDir(t *testing.T, name string, chartYaml string, valuesYaml string) string {
	...
	err = os.WriteFile(filepath.Join(chartPath, "Chart.yaml"), []byte(chartYaml), 0644)
	require.NoError(t, err, "Failed to write Chart.yaml")

	err = os.WriteFile(filepath.Join(chartPath, "values.yaml"), []byte(valuesYaml), 0644)
	require.NoError(t, err, "Failed to write values.yaml")

	// Add a dummy template file
	err = os.WriteFile(filepath.Join(chartPath, "templates", "dummy.yaml"), []byte("kind: Pod"), 0644)
