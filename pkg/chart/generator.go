...
func (g *Generator) Generate() (*override.OverrideFile, error) {
	...
	for i, img := range eligibleImages {
		...
		// --- START MODIFICATION: Determine value type based on original location ---
		var newValueToSet interface{}
		if img.LocationType == image.TypeString {
			// Original was a string, construct the new full image string
			var targetImageString string
			if img.Reference.Digest != "" {
				targetImageString = fmt.Sprintf("%s/%s@%s", g.targetRegistry, transformedRepoPath, img.Reference.Digest)
			} else {
				tag := "latest" // Default tag
				if img.Reference.Tag != "" {
					tag = img.Reference.Tag
				}
				targetImageString = fmt.Sprintf("%s/%s:%s", g.targetRegistry, transformedRepoPath, tag)
			}
			newValueToSet = targetImageString
			debug.Printf("Original was String. Setting value as string: %s", targetImageString)
		} else {
			// Original was a map (or unknown, default to map structure)
			...
			newValueToSet = imageConfig
			debug.Printf("Original was Map/Unknown. Setting value as map: %#v", imageConfig)
		}
		// --- END MODIFICATION ---

		// <<< ADD DETAILED DEBUG LOGGING HERE >>>
		// Use fmt.Printf to ensure output is captured by test runner
		fmt.Printf("[DEBUG irr GEN] Processing Eligible Image: Path=%v, OriginalRef=%s, Type=%s\n", img.Location, img.Reference.String(), locationTypeToString(img.LocationType))
		fmt.Printf("[DEBUG irr GEN] Value to set: %#v\n", newValueToSet)                   // Log the value being set
		fmt.Printf("[DEBUG irr GEN] Calling SetValueAtPath with Path: %v\n", img.Location) // Reiterate path for clarity

		// Set the value at the correct path
		err = override.SetValueAtPath(modifiedValues, img.Location, newValueToSet) // Use newValueToSet
		if err != nil {
			// Use fmt.Printf here too for consistency during debugging
			fmt.Printf("[DEBUG irr GEN] Error setting value at path %v: %v\n", img.Location, err)
		...
		if !processingFailed {
			...
			fmt.Printf("[DEBUG irr GEN] SetValueAtPath successful for path: %v\n", img.Location) // Log success
		} else {
	...
	*/
	...
	debug.DumpValue("Returning fully modified values map as overrides", modifiedValues)

	return &override.OverrideFile{
		ChartPath:   g.chartPath,
		ChartName:   filepath.Base(g.chartPath),
		Overrides:   modifiedValues, // Return the map modified in the first loop
		Unsupported: unsupported,
	}, nil
}
