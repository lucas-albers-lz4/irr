...
// sortPatternsByPath sorts ImagePatterns by path for consistent test results.
...
// findPatternByPath finds a pattern by its path in a slice of ImagePatterns.
...
func TestSimplePatternMatching(t *testing.T) {
	...
	expectedPatterns := []ImagePattern{
		{Path: "container1.image", Type: PatternTypeString, Value: "nginx:latest", Count: 1, Structure: nil},
		{Path: "container2.image", Type: PatternTypeString, Value: "quay.io/prometheus/node-exporter:v1.0.0", Count: 1, Structure: nil},
		{Path: "container3.image", Type: PatternTypeString, Value: "mysql:8.0", Count: 1, Structure: nil},
	}
	...
	for i, expected := range expectedPatterns {
		...
		if expected.Type == PatternTypeString {
...
func TestBasicValueTraversal(t *testing.T) {
	...
	expectedPatternsMap := map[string]ImagePattern{
		"mainContainer.image":    {Path: "mainContainer.image", Type: PatternTypeString, Value: "app:latest", Structure: nil},
		"initContainer.image":    {Path: "initContainer.image", Type: PatternTypeString, Value: "busybox:1.32", Structure: nil},
		"sidecarContainer.image": {Path: "sidecarContainer.image", Type: PatternTypeString, Value: "proxy:v2", Structure: nil},
		"jobTemplate.spec.image": {Path: "jobTemplate.spec.image", Type: PatternTypeString, Value: "batch-processor:stable", Structure: nil},
		"withPort.image":         {Path: "withPort.image", Type: PatternTypeString, Value: "registry.local:5000/app/service:v3", Structure: nil},
		"imageMap": {
			Path:  "imageMap",
			Type:  PatternTypeMap,
			Value: "repository=map-repo,registry=explicit.registry,tag=map-tag",
			Structure: map[string]interface{}{
				keys.Registry:   "explicit.registry",
				keys.Repository: "map-repo",
				keys.Tag:        "map-tag",
			},
		},
		"nestedMap.innerImage.image": {Path: "nestedMap.innerImage.image", Type: PatternTypeString, Value: "nested:abc", Structure: nil},
		"mapInArray[0].image":        {Path: "mapInArray[0].image", Type: PatternTypeString, Value: "map-array-img:tagC", Structure: nil},
		"mapWithImageKey.image":      {Path: "mapWithImageKey.image", Type: PatternTypeString, Value: "key-implies-image:tagD", Structure: nil},
		"mapWithRepositoryKey": {
			...
			Structure: map[string]interface{}{
				...
				keys.Repository: "library/key-implies-repo",
				keys.Tag:        "tagE",
			},
		},
	}
	...
	assert.False(t, foundStringWithTag, "Pattern 'stringWithRegistryAndTag' should NOT be found")

	_, foundArrayImage := findPatternByPath(patterns, "imageInArray[0]")
	assert.False(t, foundArrayImage, "Pattern 'imageInArray[0]' should NOT be found")

	_, foundConfigMapURL := findPatternByPath(patterns, "configMapData.imageUrl")
	assert.False(t, foundConfigMapURL, "Pattern 'configMapData.imageUrl' should NOT be found")
	...
	assert.Equal(t, PatternTypeMap, mapRepoPattern.Type)
...
func TestRecursiveAnalysisWithNestedStructures(t *testing.T) {
	...
	expectedPatternsMap := map[string]ImagePattern{
		"level1.image":               {Path: "level1.image", Type: PatternTypeString, Value: "image1:v1", Structure: nil},
		"level1.level2.image":        {Path: "level1.level2.image", Type: PatternTypeString, Value: "image2:v2", Structure: nil},
		"level1.level2.level3.image": {Path: "level1.level2.level3.image", Type: PatternTypeString, Value: "image3:v3", Structure: nil},
		"unrelated.image":            {Path: "unrelated.image", Type: PatternTypeString, Value: "unrelated:abc", Structure: nil},
		"mapImage": {
			Path:  "mapImage",
			Type:  PatternTypeMap,
			Value: "repository=repo1,registry=reg.com,tag=t1",
			Structure: map[string]interface{}{
				keys.Registry:   "reg.com",
				keys.Repository: "repo1",
				keys.Tag:        "t1",
			},
		},
		"nestedMapImage.inner": {
			...
			Structure: map[string]interface{}{
				keys.Registry:   "nested.reg",
				keys.Repository: "nested-repo",
				keys.Tag:        "t2",
			},
		},
	}
...
func TestAggregatePatterns(t *testing.T) {
	patterns := []ImagePattern{
		{Path: "image", Type: PatternTypeString, Value: "nginx:latest", Count: 1},
		{Path: "image", Type: PatternTypeString, Value: "nginx:latest", Count: 1},
		{Path: "other.image", Type: PatternTypeString, Value: "redis:alpine", Count: 1},
		{Path: "service.image", Type: PatternTypeMap, Value: "mysql:8.0", Count: 1},
		{Path: "service.image", Type: PatternTypeMap, Value: "mysql:8.0", Count: 1},
		{Path: "service.image", Type: PatternTypeMap, Value: "mysql:8.0", Count: 1},
	}

	expected := []ImagePattern{
		{Path: "image", Type: PatternTypeString, Value: "nginx:latest", Count: 2},
		{Path: "other.image", Type: PatternTypeString, Value: "redis:alpine", Count: 1},
		{Path: "service.image", Type: PatternTypeMap, Value: "mysql:8.0", Count: 3},
	}
	...
	testCases := []struct {
	...
	}{
		{
			...
			checkPattern: func(t *testing.T, p ImagePattern) {
				...
				assert.Equal(t, PatternTypeString, p.Type)
		...
		},
		{
			...
			checkPattern: func(t *testing.T, p ImagePattern) {
				assert.Equal(t, "image", p.Path)
				assert.Equal(t, PatternTypeMap, p.Type)
				require.NotNil(t, p.Structure)
				assert.Equal(t, "docker.io", p.Structure[keys.Registry])
				assert.Equal(t, "library/nginx", p.Structure[keys.Repository])
				assert.Equal(t, "latest", p.Structure[keys.Tag])
			},
		},
		{
