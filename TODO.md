...
Hints from our previous failed attempt to refactor this:
Core Issue: The primary driver for test failures is the removal of the legacy registry format support in Phase 5. Tests written assuming the old format or its conversion logic will break.
config_test.go Needs Overhaul: This file likely requires the most significant changes. Expect to fix:
Incorrect type references (e.g., use BitnamiCompatibility instead of the old BitnamiCompatConfig).
Outdated function calls (e.g., ensuring validateStructuredConfig is called correctly).
Incorrect mock filesystem usage (needs to align with fileutil.NewMockFS or fileutil.NewAferoFS).
Removal of tests relying on legacy format conversion or loading.
Function Name Clash: The function TestGetDefaultMappingsPath is declared in both config_default_test.go and mappings_default_test.go. One of these will need to be renamed or removed to resolve the conflict.
Type Mismatch in mappings_test.go: Calls to registry loading functions (like the old LoadMappings or the current LoadConfigWithFS) need to receive a fileutil.FS interface, not a raw afero.Fs. Ensure the afero.Fs mock is wrapped using fileutil.NewAferoFS().


#### Phase 5.1: Identify & Analyze Legacy Usage
- [x] **[P1]** **Code Search:** Perform a codebase-wide search (using `grep` or IDE tools) for:
...
- [x] **[P1]** **List Affected Files:** Compile a list of all `.go` files (implementation and tests) identified in the search. This forms the basis for the refactoring work. Key areas likely include:
...
- [x] **[P1]** **Analysis Notes (Test Impact):**
...
- [x] **[P1]** **Summarize Findings:** Compile a checklist or summary of specific code locations and required changes before proceeding to Phase 5.2.
    - **Detailed Findings (2024-07-29):**
        - **Core Legacy Logic/Conversion:**
            - `pkg/registry/config.go`: `ConvertToLegacyFormat` definition. Functions `LoadConfigWithFS` and `LoadConfigDefault` *return* `map[string]string`.
            - `pkg/registry/mappings.go`: Functions `LoadConfig`, `tryLoadLegacyConfig`, `validateLegacyMappings` handle or return `map[string]string`. `ConvertToLegacyFormat` is called internally as part of fallback logic in `LoadMappings`. `EmptyPathResult` uses legacy type.
        - **Direct Usage in Commands:**
            - `cmd/irr/override.go`: `OverrideCommand` struct has `ConfigMappings map[string]string` field.
            - Note: `inspect` and `config` commands need review during Phase 5.3, but don't appear to store mappings in legacy format fields directly.
        - **Test Infrastructure:**
            - `test/integration/harness.go`: `CreateRegistryMappingsFile` generates legacy `map[string]string` and writes it using `yaml.Marshal`, effectively creating legacy format files for tests.
        - **Test Assertions/Setup Using Legacy Format:**
            - `pkg/registry/config_default_test.go`: Uses `ConvertToLegacyFormat` for comparisons, contains fallback test `TestLoadMappingsDefault_FallbackToLegacy`.
            - `pkg/registry/config_test.go`: Uses `ConvertToLegacyFormat` for assertions, test helper `setupConfigTestFiles` returns legacy map, test structs contain legacy map fields.
            - `pkg/registry/fs_test.go`: Uses `ConvertToLegacyFormat`, contains fallback test `TestLoadMappingsWithFS_FallbackToLegacy`.
            - `cmd/irr/override_test.go`: Test setup initializes `ConfigMappings` field with a literal legacy map.
            - `test/integration/registry_test.go` (and potentially others): Likely relies on the harness generating legacy files and tests legacy loading behavior. Needs explicit review in Phase 5.3.

#### Phase 5.2: Refactor Core Registry Package
- [x] **[P1]** **Remove Conversion:** Delete the `ConvertToLegacyFormat` function from `pkg/registry/mappings.go`.
...
- [x] **[P1]** **Update Signatures:** Change any function signatures currently accepting `map[string]string` for registries to exclusively accept `*registry.Config` or related structured types.
- [x] **[P1]** **Clean Up Tests:** Update tests within `pkg/registry/` to remove assertions related to legacy format conversion or loading. Ensure tests focus solely on the structured format.
- [x] **[P1]** **Verify:** Run `make lint` and `go test ./pkg/registry/...` to ensure the package is clean and tests pass.
