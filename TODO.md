...
## Phase 11: Refactor Override Output Writing for Afero Compliance

### Overview
Modify the `override` command's output logic to use the `afero.Fs` interface instead of direct `os.WriteFile` calls. This aligns the output handling with the project's filesystem mocking strategy defined in `docs/FILESYSTEM-MOCKING.md`.

### Motivation
- **Consistency:** Ensure all IRR-managed filesystem operations (where feasible) use the `afero` abstraction.
- **Testability:** Enable straightforward unit testing of the output writing logic using `afero.MemMapFs`, independent of the real filesystem.
- **Maintainability:** Centralize filesystem interactions through the `afero.Fs` interface, simplifying future refactoring or behavior changes.
- **Potential Reliability:** Using a consistent abstraction might help mitigate rare filesystem inconsistencies.

### Implementation Steps

- [ ] **[P1]** **Modify `outputOverrides` Signature:**
    - Change `func outputOverrides(cmd *cobra.Command, yamlBytes []byte, outputFile string, dryRun bool) error`
    - To `func outputOverrides(cmd *cobra.Command, fs afero.Fs, yamlBytes []byte, outputFile string, dryRun bool) error`
- [ ] **[P1]** **Update Path Validation within `outputOverrides`:**
    - Review validation logic before the write call (around lines 400-417).
    - Replace `os.Stat` calls with `fs.Stat`.
    - Ensure helpers like `fileutil.ValidateOutputPath` and `fileutil.EnsureCorrectPermissions` receive and use the `fs afero.Fs` instance.
- [ ] **[P1]** **Replace `os.WriteFile`:**
    - In `outputOverrides` (around line 418), replace `os.WriteFile(absPath, yamlBytes, 0644)` with `fs.WriteFile(absPath, yamlBytes, 0644)`.
- [ ] **[P1]** **Update Caller (`runOverride`):**
    - In `runOverride` (and potentially helpers like `handlePluginOverrideOutput`), instantiate `fs := afero.NewOsFs()` before calling `outputOverrides`.
    - Pass the `fs` instance when calling `outputOverrides`.
- [ ] **[P1]** **Add Unit Tests:**
    - Create/update unit tests for `outputOverrides` in `cmd/irr/`.
    - Use `mockFs := afero.NewMemMapFs()`.
    - Call `outputOverrides` with `mockFs`.
    - Assert file content on `mockFs` using `afero.ReadFile`.
    - Test success and potential error cases (e.g., invalid path).
- [ ] **[P1]** **Verify Integration Tests:**
    - Run the full integration test suite (`make test` or similar).
    - Confirm all tests involving `override` command file output still pass (they will now use `afero.OsFs`).
- [ ] **[P2]** **Update Documentation:**
    - Note the adoption of `afero` for output writing in `cmd/irr/override.go` in `docs/FILESYSTEM-MOCKING.md` or developer notes.

### Acceptance Criteria
- The `override` command uses `afero.Fs.WriteFile` to write the output file when `--output-file` is specified.
- Direct calls to `os.WriteFile` for the output file are removed from `cmd/irr/override.go`.
- Unit tests specifically verify the `outputOverrides` function using `afero.MemMapFs`.
- All existing integration tests continue to pass.
- Filesystem interactions within `outputOverrides` consistently use the provided `afero.Fs` instance.

### Testing Strategy
- Add unit tests focusing solely on the `outputOverrides` function's interaction with `afero.Fs` (using `MemMapFs`).
- Rely on existing integration tests to verify the end-to-end behavior with the real filesystem (`OsFs`).
- Test edge cases in unit tests, such as empty file paths or empty content.